				...
				<div class="card">
					<div class="card-content">
						<div class="row">
							<div class="col-md-2">
								<h5>飯店名稱</h5>
								<input style="margin-top: 15px;" type="text" name="hotelname"><button class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect" type="button" name="send" id="buttonsend">搜尋</button>
...
$(function(){
	...
	$('#buttonsend').on('click', function(){
		...
		if(hotelname != ''){
			...
			var tbody = $('#room');
			var rtidbody = $('#roomTypeId');
			var opNon = $('<option></option>').text('--')
			rtidbody.empty().append(opNon);
			tbody.empty();
			
			$.ajax({
			type : 'GET',
			url : '${pageContext.servletContext.contextPath}/manager/HotelRoomTypeForManagerServlet',
			data : {
				hotelname : hotelname
			},
			success : function(data){
				var opAll = $('<option></option>').val('all').text('全部')
				rtidbody.empty().append(opAll);
				
				var hiddendata = data.hiddenid;
				$('input[name="hiddenid"]').val(hiddendata);
				
				var rtAll = data.roomTypeAll;
				$.each(rtAll, function(name, value){
					var opt = $('<option></option>').val(value.rtid).text(value.rtn);
					
					rtidbody.append(opt);
				})
				
				var records = data.hotelRecord;
				$.each(records, function(i,record) {
					console.log(records[i]);
					gentr(i,records[i],tbody);
				})
			}
		})
		}
	...
	$('#roomTypeId').change(function(){
		...
		if(id == 'all'){
			var hiddenid = $('input[name="hiddenid"]').val();
			$.getJSON("${pageContext.servletContext.contextPath}/json/HotelIdRecordToJson",{"hotelId":hiddenid},
					function(data){
					var tbody = $('#room');
					tbody.empty();
			...
			var hiddenid = $('input[name="hiddenid"]').val();
			$.getJSON("${pageContext.servletContext.contextPath}/json/RoomTypeRecordToJosn",{"roomTypeId":id,"hiddenid":hiddenid},
					function(data){
					var tbody = $('#room');
					tbody.empty();
				$.each(data, function(i,record){
